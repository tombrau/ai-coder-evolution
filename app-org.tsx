...
interface LogDataPoint extends DataPoint {
  logCapability: string;
}

interface ProgrammerLevel {
  year: string;
  level: string;
  capability: number;
}
...
const timelineData: DataPoint[] = [...historicalData, ...projectedData];

// Calculate logarithmic values (base 10) for the second graph
const logData: LogDataPoint[] = timelineData.map(item => ({
  ...item,
...
const programmerLevels: ProgrammerLevel[] = [
...
const CustomizedDot: React.FC<CustomDotProps> = (props) => {
  ...
  const isNotable = payload?.label || (payload?.milestone && payload.milestone.includes("PROGRAMMER"));
...
// Main component
const AICodeTimeline: React.FC = () => {
  // A more extreme value for Y-axis to accommodate the quantum acceleration
  const linearYAxisDomain: [number, number] = [0, 50000000000];
