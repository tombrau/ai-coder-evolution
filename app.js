...
// Then use these types in your component definitions
var CustomTooltip = function (_a) {
    ...
    var LineChart = Recharts.LineChart, Line = Recharts.Line, XAxis = Recharts.XAxis, YAxis = Recharts.YAxis, CartesianGrid = Recharts.CartesianGrid, Tooltip = Recharts.Tooltip, Legend = Recharts.Legend, ResponsiveContainer = Recharts.ResponsiveContainer, Label = Recharts.Label, ReferenceLine = Recharts.ReferenceLine;
    ...
    var timelineData = __spreadArray(__spreadArray([], historicalData, true), projectedData, true);
    // Calculate logarithmic values (base 10) for the second graph
    var logData = timelineData.map(function (item) { return (__assign(__assign({}, item), { logCapability: Math.log10(item.capability).toFixed(2) })); });
    // Update programmer levels based on the dramatically accelerated growth projection
    var programmerLevels = [
    ...
    var CustomizedDot = function (props) {
        ...
        var isNotable = payload.label || payload.milestone.includes("PROGRAMMER");
        if (isNotable) {
            return (React.createElement("circle", { cx: cx, cy: cy, r: 4, stroke: "red", strokeWidth: 1, fill: "red" }));
        }
        return (React.createElement("circle", { cx: cx, cy: cy, r: 2, stroke: "none", fill: "#8884d8" }));
    };
    // Main component
    var AICodeTimeline = function () {
        // A more extreme value for Y-axis to accommodate the quantum acceleration
        var linearYAxisDomain = [0, 50000000000];
                ...
                React.createElement("div", { className: "h-96" },
                    React.createElement(ResponsiveContainer, { width: "100%", height: "100%" },
                        React.createElement(LineChart, { data: timelineData, margin: { top: 20, right: 30, left: 30, bottom: 70 } },
                            ...
                            programmerLevels.map(function (level) { return (React.createElement(ReferenceLine, { key: level.level, y: level.capability, stroke: "green", strokeDasharray: "3 3", label: {
                                    value: level.level,
                                    position: 'right',
                                    fill: 'green',
                                    fontSize: 12
                ...
                React.createElement("div", { className: "h-96" },
                    React.createElement(ResponsiveContainer, { width: "100%", height: "100%" },
                        React.createElement(LineChart, { data: logData, margin: { top: 20, right: 30, left: 30, bottom: 70 } },
                            ...
                            programmerLevels.map(function (level) { return (React.createElement(ReferenceLine, { key: level.level, y: Math.log10(level.capability).toFixed(2), stroke: "green", strokeDasharray: "3 3", label: {
                                    value: level.level,
                                    position: 'right',
                                    fill: 'green',
                                    fontSize: 12
    ...
    ReactDOM.render(React.createElement(AICodeTimeline, null), document.getElementById('root'));
};
